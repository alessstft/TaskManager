...
class PerformanceTab(tk.Frame):
    ...
    def switch_metric(self, metric):
        ...
        # Обновляем информацию о памяти, если это переключение на вкладку памяти
        if metric == 'memory':
            # Метрика памяти, обновляем детализацию
            self._update_memory_details(self._last_system_info if hasattr(self, '_last_system_info') else {})
        else:
            # Скрываем детали памяти для других вкладок
            if hasattr(self, 'memory_details'):
                self.memory_details.place_forget()
    ...
    def _update_memory_details(self, system_info):
        ...
        if not hasattr(self, 'memory_details'):
            # Создаем контейнер для деталей памяти
            parent = self.winfo_toplevel()
            self.memory_details = tk.Frame(parent, bg='#1e1e1e', highlightbackground="#3c3c3c", highlightthickness=1)
            
            # Размещаем под графиком
            self.memory_details.place(x=240, y=350, width=600, height=200)
            
            # Заголовок для панели деталей памяти
            header = tk.Label(self.memory_details, text="Использование памяти", font=("Arial", 12, "bold"), 
                                bg="#1e1e1e", fg="white")
            header.pack(anchor="w", padx=10, pady=(15, 1))
            
            # Контейнер для меток
            details_container = tk.Frame(self.memory_details, bg='#1e1e1e')
            details_container.pack(fill="both", expand=True, padx=5)
            
            # Создаем метки для данных о памяти
            labels = [
                ("Используется (сжатая)", "6,4 ГБ (188 МБ)"),
                ("Доступно", "1,4 ГБ"),
                ("Скорость:", "1600 МГц"),
                ("Выделено", "10,0/12,6 ГБ"),
                ("Кэшировано", "1,1 ГБ"),
            ...
            # Создаем метки с данными о памяти
            self.memory_detail_labels = {}
            for i, (name, value) in enumerate(labels):
                row = i // 2
                col = i % 2
                frame = tk.Frame(details_container, bg='#1e1e1e')
                ...
                self.memory_detail_labels[name] = tk.Label(frame, text=value, bg='#1e1e1e', fg='white', anchor='w', width=25)
                self.memory_detail_labels[name].pack(side='top', fill='x')
        else:
            self.memory_details.lift()
            
        # При желании здесь можно обновить значения из self.system_info
